package com.boydguy.backend.dao.base;
...
public abstract class BaseDao<T extends Serializable> {
    ...
    @ClearAppCache(methodName = "select", paramTypes = {"T"})
    public int delete(T record) {
        return mapper.delete(record);
    }

    // 根据主键进行删除，查询条件使用等号
    @ClearAppCache(methodName = "selectByPrimaryKey", paramTypes = {"java.lang.Object"})
    ...
    @ClearAppCache(methodName = "selectByIds", paramTypes = {"java.lang.String"})
    ...
    @ClearAppCache(methodName = "selectByExample", paramTypes = {"tk.mybatis.mapper.entity.Example"})
    public int deleteByExample(Example example) {
        return mapper.deleteByExample(example);
    }

    // 根据Example条件更新实体`record`包含的全部属性，null值会被更新，返回更新的条数
    @ClearAppCache(methodName = "selectByExample", paramTypes = {"tk.mybatis.mapper.entity.Example"}, index = 1)
